...
async def set_username(message, state):
    if message.text in ['Норма калорий?', 'Регистрация', 'Купить', 'Информация']:
        await state.finish()
...
async def set_email(message, state):
    if message.text in ['Норма калорий?', 'Регистрация', 'Купить', 'Информация']:
        ...
        await message.answer(text='Введите полное количество лет:')
...
async def set_age(message, state):
    if message.text in ['Норма калорий?', 'Регистрация', 'Купить', 'Информация']:
        ...
        await message.answer(text='Регистрация выполнена успешно!')

@dp.message_handler(text = 'Купить')
async def get_buying_list(message):
    for product in products_data:
        with open(f'{product[0]}.png', 'rb') as photo:
            await message.answer_photo(photo,
            f'Название: {product[1]} |\n'
            f'Описание: {product[2]} |\n'
            f'Цена: {product[3]} рублей за штуку.')
...
async def send_calories(message, state):
    ...
    data = await state.get_data()
    try:
        if data['sex'].lower() == 'м' or 'v':
            calories = ((10 * float(data['weight']) + 6.25 * float(data['growth']) - 5 * float(data['age']) + 5)
                        * ACTIVITY[data['activity']])
        elif data['sex'].lower() == 'ж' or ';' or ':':
            ...
            calories = 0
    await message.answer(f'Ваша суточная норма килокалорий: {calories:.{0}f}')
    if calories == 0:
